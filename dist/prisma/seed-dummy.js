"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g = Object.create((typeof Iterator === "function" ? Iterator : Object).prototype);
    return g.next = verb(0), g["throw"] = verb(1), g["return"] = verb(2), typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
var client_1 = require("@prisma/client");
var library_1 = require("@prisma/client/runtime/library");
var prisma = new client_1.PrismaClient();
function main() {
    return __awaiter(this, void 0, void 0, function () {
        var arizaTipleri, uzmanlikAlanlari, malzemeler, projeler, bloklar, _a, _b, _c, daireler, _d, _e, _f, teknikerler, arizalar, _g, _h, _j, randevular, _k, _l, _m, randevuMalzemeler, _o, _p, _q;
        var _r, _s, _t, _u, _v, _w, _x, _y, _z, _0, _1, _2, _3, _4, _5, _6, _7;
        var _this = this;
        var _8, _9, _10, _11, _12, _13, _14, _15, _16, _17, _18, _19, _20, _21, _22;
        return __generator(this, function (_23) {
            switch (_23.label) {
                case 0:
                    console.log('Starting seed...');
                    // Ariza Tipleri (Fault Types)
                    console.log('Creating ArizaTipleri...');
                    return [4 /*yield*/, prisma.arizaTipi.createMany({
                            data: [
                                { ad: 'Su', ekbilgi: 'Su tesisatı arızaları' },
                                { ad: 'Elektrik', ekbilgi: 'Elektrik tesisatı arızaları' },
                                { ad: 'Isıtma', ekbilgi: 'Isıtma sistemi arızaları' },
                                { ad: 'Kapı', ekbilgi: 'Kapı ve kilit arızaları' },
                                { ad: 'Asansör', ekbilgi: 'Asansör arızaları' },
                                { ad: 'Diğer', ekbilgi: 'Diğer arızalar' },
                            ],
                            skipDuplicates: true,
                        })];
                case 1:
                    arizaTipleri = _23.sent();
                    console.log({ arizaTipleri: arizaTipleri });
                    console.log('ArizaTipleri created.');
                    // Uzmanlik Alanlari (Expertise Areas)
                    console.log('Creating UzmanlikAlanlari...');
                    return [4 /*yield*/, prisma.uzmanlikAlani.createMany({
                            data: [
                                { ad: 'Su Tesisatı', ekbilgi: 'Su tesisatı uzmanlığı' },
                                { ad: 'Elektrik Tesisatı', ekbilgi: 'Elektrik tesisatı uzmanlığı' },
                                { ad: 'Isıtma Sistemleri', ekbilgi: 'Isıtma sistemleri uzmanlığı' },
                                { ad: 'Kapı ve Kilit', ekbilgi: 'Kapı ve kilit sistemleri uzmanlığı' },
                                { ad: 'Asansör Bakımı', ekbilgi: 'Asansör bakım ve onarım uzmanlığı' },
                                { ad: 'Genel Bakım', ekbilgi: 'Genel bakım ve onarım uzmanlığı' },
                            ],
                            skipDuplicates: true,
                        })];
                case 2:
                    uzmanlikAlanlari = _23.sent();
                    console.log({ uzmanlikAlanlari: uzmanlikAlanlari });
                    console.log('UzmanlikAlanlari created.');
                    // Malzemeler (Materials)
                    console.log('Creating Malzemeler...');
                    return [4 /*yield*/, prisma.malzeme.createMany({
                            data: [
                                { ad: 'Su Borusu', birim: 'Metre' },
                                { ad: 'Kablo', birim: 'Metre' },
                                { ad: 'Radyatör', birim: 'Adet' },
                                { ad: 'Kilit', birim: 'Adet' },
                                { ad: 'Asansör Halatı', birim: 'Metre' },
                                { ad: 'Vida', birim: 'Adet' },
                            ],
                            skipDuplicates: true,
                        })];
                case 3:
                    malzemeler = _23.sent();
                    console.log({ malzemeler: malzemeler });
                    console.log('Malzemeler created.');
                    // Projeler (Projects)
                    console.log('Creating Projeler...');
                    return [4 /*yield*/, prisma.proje.createMany({
                            data: [
                                { ad: 'Güneş Sitesi', ekbilgi: 'Güneş Sitesi Projesi', adres: 'İstanbul' },
                                { ad: 'Ay Sitesi', ekbilgi: 'Ay Sitesi Projesi', adres: 'Ankara' },
                                { ad: 'Yıldız Sitesi', ekbilgi: 'Yıldız Sitesi Projesi', adres: 'İzmir' },
                            ],
                            skipDuplicates: true,
                        })];
                case 4:
                    projeler = _23.sent();
                    console.log({ projeler: projeler });
                    console.log('Projeler created.');
                    // Bloklar (Blocks)
                    console.log('Creating Bloklar...');
                    _b = (_a = prisma.blok).createMany;
                    _r = {};
                    _s = { ad: 'A Blok' };
                    return [4 /*yield*/, (function () { return __awaiter(_this, void 0, void 0, function () { var proje, error_1; return __generator(this, function (_a) {
                            switch (_a.label) {
                                case 0:
                                    _a.trys.push([0, 2, , 3]);
                                    return [4 /*yield*/, prisma.proje.findFirst({ where: { ad: 'Güneş Sitesi' } })];
                                case 1:
                                    proje = _a.sent();
                                    if (!proje) {
                                        throw new Error("Proje not found");
                                    }
                                    return [2 /*return*/, proje.id];
                                case 2:
                                    error_1 = _a.sent();
                                    console.error("Error finding Proje:", error_1);
                                    throw error_1;
                                case 3: return [2 /*return*/];
                            }
                        }); }); })()];
                case 5:
                    _c = [
                        (_s.projeId = _23.sent(), _s)
                    ];
                    _t = { ad: 'B Blok' };
                    return [4 /*yield*/, (function () { return __awaiter(_this, void 0, void 0, function () { var proje, error_2; return __generator(this, function (_a) {
                            switch (_a.label) {
                                case 0:
                                    _a.trys.push([0, 2, , 3]);
                                    return [4 /*yield*/, prisma.proje.findFirst({ where: { ad: 'Güneş Sitesi' } })];
                                case 1:
                                    proje = _a.sent();
                                    if (!proje) {
                                        throw new Error("Proje not found");
                                    }
                                    return [2 /*return*/, proje.id];
                                case 2:
                                    error_2 = _a.sent();
                                    console.error("Error finding Proje:", error_2);
                                    throw error_2;
                                case 3: return [2 /*return*/];
                            }
                        }); }); })()];
                case 6:
                    _c = _c.concat([
                        (_t.projeId = _23.sent(), _t)
                    ]);
                    _u = { ad: 'C Blok' };
                    return [4 /*yield*/, (function () { return __awaiter(_this, void 0, void 0, function () { var proje, error_3; return __generator(this, function (_a) {
                            switch (_a.label) {
                                case 0:
                                    _a.trys.push([0, 2, , 3]);
                                    return [4 /*yield*/, prisma.proje.findFirst({ where: { ad: 'Ay Sitesi' } })];
                                case 1:
                                    proje = _a.sent();
                                    if (!proje) {
                                        throw new Error("Proje not found");
                                    }
                                    return [2 /*return*/, proje.id];
                                case 2:
                                    error_3 = _a.sent();
                                    console.error("Error finding Proje:", error_3);
                                    throw error_3;
                                case 3: return [2 /*return*/];
                            }
                        }); }); })()];
                case 7: return [4 /*yield*/, _b.apply(_a, [(_r.data = _c.concat([
                            (_u.projeId = _23.sent(), _u)
                        ]),
                            _r.skipDuplicates = true,
                            _r)])];
                case 8:
                    bloklar = _23.sent();
                    console.log({ bloklar: bloklar });
                    console.log('Bloklar created.');
                    // Daireler (Apartments)
                    console.log('Creating Daireler...');
                    _e = (_d = prisma.daire).createMany;
                    _v = {};
                    _w = { numara: '1', kat: '1' };
                    return [4 /*yield*/, prisma.blok.findFirst({ where: { ad: 'A Blok' } })];
                case 9:
                    _f = [
                        (_w.blokId = ((_8 = (_23.sent())) === null || _8 === void 0 ? void 0 : _8.id) || '', _w)
                    ];
                    _x = { numara: '2', kat: '1' };
                    return [4 /*yield*/, prisma.blok.findFirst({ where: { ad: 'A Blok' } })];
                case 10:
                    _f = _f.concat([
                        (_x.blokId = ((_9 = (_23.sent())) === null || _9 === void 0 ? void 0 : _9.id) || '', _x)
                    ]);
                    _y = { numara: '3', kat: '2' };
                    return [4 /*yield*/, prisma.blok.findFirst({ where: { ad: 'B Blok' } })];
                case 11: return [4 /*yield*/, _e.apply(_d, [(_v.data = _f.concat([
                            (_y.blokId = ((_10 = (_23.sent())) === null || _10 === void 0 ? void 0 : _10.id) || '', _y)
                        ]),
                            _v.skipDuplicates = true,
                            _v)])];
                case 12:
                    daireler = _23.sent();
                    console.log({ daireler: daireler });
                    console.log('Daireler created.');
                    // Teknikerler (Technicians)
                    console.log('Creating Teknikerler...');
                    return [4 /*yield*/, prisma.tekniker.createMany({
                            data: [
                                { adsoyad: 'Ahmet Yılmaz' },
                                { adsoyad: 'Ayşe Demir' },
                            ],
                            skipDuplicates: true,
                        })];
                case 13:
                    teknikerler = _23.sent();
                    console.log({ teknikerler: teknikerler });
                    console.log('Teknikerler created.');
                    // Arizalar (Faults)
                    console.log('Creating Arizalar...');
                    _h = (_g = prisma.ariza).createMany;
                    _z = {};
                    _0 = {
                        aciklama: 'Su sızıntısı var'
                    };
                    return [4 /*yield*/, prisma.arizaTipi.findFirst({ where: { ad: 'Su' } })];
                case 14:
                    _0.arizaTipiId = ((_11 = (_23.sent())) === null || _11 === void 0 ? void 0 : _11.id) || '';
                    return [4 /*yield*/, prisma.daire.findFirst({ where: { numara: '1' } })];
                case 15:
                    _j = [
                        (_0.daireId = ((_12 = (_23.sent())) === null || _12 === void 0 ? void 0 : _12.id) || '',
                            _0.durum = 'Talep Alındı',
                            _0.createdAt = new Date(),
                            _0)
                    ];
                    _1 = {
                        aciklama: 'Elektrik kesintisi var'
                    };
                    return [4 /*yield*/, prisma.arizaTipi.findFirst({ where: { ad: 'Elektrik' } })];
                case 16:
                    _1.arizaTipiId = ((_13 = (_23.sent())) === null || _13 === void 0 ? void 0 : _13.id) || '';
                    return [4 /*yield*/, prisma.daire.findFirst({ where: { numara: '2' } })];
                case 17: return [4 /*yield*/, _h.apply(_g, [(_z.data = _j.concat([
                            (_1.daireId = ((_14 = (_23.sent())) === null || _14 === void 0 ? void 0 : _14.id) || '',
                                _1.durum = 'Randevu Planlandı',
                                _1.createdAt = new Date(),
                                _1)
                        ]),
                            _z.skipDuplicates = true,
                            _z)])];
                case 18:
                    arizalar = _23.sent();
                    console.log({ arizalar: arizalar });
                    console.log('Arizalar created.');
                    // Randevular (Appointments)
                    console.log('Creating Randevular...');
                    _l = (_k = prisma.randevu).createMany;
                    _2 = {};
                    _3 = {
                        tarih: new Date()
                    };
                    return [4 /*yield*/, prisma.ariza.findFirst({ where: { aciklama: 'Su sızıntısı var' } })];
                case 19:
                    _3.arizaId = ((_15 = (_23.sent())) === null || _15 === void 0 ? void 0 : _15.id) || '';
                    return [4 /*yield*/, prisma.tekniker.findFirst({ where: { adsoyad: 'Ahmet Yılmaz' } })];
                case 20:
                    _m = [
                        (_3.teknikerId = ((_16 = (_23.sent())) === null || _16 === void 0 ? void 0 : _16.id) || '',
                            _3.notlar = 'Su sızıntısı kontrolü yapılacak',
                            _3.durum = 'Planlandı',
                            _3)
                    ];
                    _4 = {
                        tarih: new Date()
                    };
                    return [4 /*yield*/, prisma.ariza.findFirst({ where: { aciklama: 'Elektrik kesintisi var' } })];
                case 21:
                    _4.arizaId = ((_17 = (_23.sent())) === null || _17 === void 0 ? void 0 : _17.id) || '';
                    return [4 /*yield*/, prisma.tekniker.findFirst({ where: { adsoyad: 'Ayşe Demir' } })];
                case 22: return [4 /*yield*/, _l.apply(_k, [(_2.data = _m.concat([
                            (_4.teknikerId = ((_18 = (_23.sent())) === null || _18 === void 0 ? void 0 : _18.id) || '',
                                _4.notlar = 'Elektrik tesisatı kontrolü yapılacak',
                                _4.durum = 'Planlandı',
                                _4)
                        ]),
                            _2.skipDuplicates = true,
                            _2)])];
                case 23:
                    randevular = _23.sent();
                    console.log({ randevular: randevular });
                    console.log('Randevular created.');
                    // RandevuMalzeme (Appointment Materials)
                    console.log('Creating RandevuMalzemeler...');
                    _p = (_o = prisma.randevuMalzeme).createMany;
                    _5 = {};
                    _6 = {};
                    return [4 /*yield*/, prisma.randevu.findFirst({ where: { notlar: 'Su sızıntısı kontrolü yapılacak' } })];
                case 24:
                    _6.randevuId = ((_19 = (_23.sent())) === null || _19 === void 0 ? void 0 : _19.id) || '';
                    return [4 /*yield*/, prisma.malzeme.findFirst({ where: { ad: 'Su Borusu' } })];
                case 25:
                    _q = [
                        (_6.malzemeId = ((_20 = (_23.sent())) === null || _20 === void 0 ? void 0 : _20.id) || '',
                            _6.miktar = 2,
                            _6.fiyat = new library_1.Decimal(100),
                            _6)
                    ];
                    _7 = {};
                    return [4 /*yield*/, prisma.randevu.findFirst({ where: { notlar: 'Elektrik tesisatı kontrolü yapılacak' } })];
                case 26:
                    _7.randevuId = ((_21 = (_23.sent())) === null || _21 === void 0 ? void 0 : _21.id) || '';
                    return [4 /*yield*/, prisma.malzeme.findFirst({ where: { ad: 'Kablo' } })];
                case 27: return [4 /*yield*/, _p.apply(_o, [(_5.data = _q.concat([
                            (_7.malzemeId = ((_22 = (_23.sent())) === null || _22 === void 0 ? void 0 : _22.id) || '',
                                _7.miktar = 5,
                                _7.fiyat = new library_1.Decimal(150),
                                _7)
                        ]),
                            _5.skipDuplicates = true,
                            _5)])];
                case 28:
                    randevuMalzemeler = _23.sent();
                    console.log({ randevuMalzemeler: randevuMalzemeler });
                    console.log('RandevuMalzemeler created.');
                    return [2 /*return*/];
            }
        });
    });
}
function runSeed() {
    return __awaiter(this, void 0, void 0, function () {
        var error_4;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    _a.trys.push([0, 2, , 3]);
                    return [4 /*yield*/, main()];
                case 1:
                    _a.sent();
                    return [3 /*break*/, 3];
                case 2:
                    error_4 = _a.sent();
                    console.error('Failed to seed the database:');
                    console.error(error_4);
                    return [3 /*break*/, 3];
                case 3: return [2 /*return*/];
            }
        });
    });
}
runSeed()
    .then(function () { return __awaiter(void 0, void 0, void 0, function () {
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0: return [4 /*yield*/, prisma.$disconnect()];
            case 1:
                _a.sent();
                return [2 /*return*/];
        }
    });
}); })
    .catch(function (e) { return __awaiter(void 0, void 0, void 0, function () {
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                console.error(e);
                return [4 /*yield*/, prisma.$disconnect()];
            case 1:
                _a.sent();
                process.exit(1);
                return [2 /*return*/];
        }
    });
}); });
